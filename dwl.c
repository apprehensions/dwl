...
struct Monitor {
	...
	struct wlr_scene_buffer *scene_buffer; /* See drawbar() for info */
...
drawbar(Monitor *m)
{
	...
	fprintf(stderr, "bar: %d %d %d\n", showbar, !m, !m->showbar);

	if (!m || !m->showbar)
		return;

	width = m->wlr_output->width;
...
setup(void)
{
	...
	if (!(font = fcft_from_name(LENGTH(fonts), fonts, NULL)))
